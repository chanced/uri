...
type URITest struct {
	...
	roundtrip string // expected result of reserializing the URI; empty means same as "in".
}
...
var urltests = []URITest{
	...
	// different URI parsing context, but currently shares the
	// same codepath)
	{
		"///threeslashes",
		&URI{
...
var parseRequestURITests = []struct {
	url           string
	expectedValid bool
}{
	{"http://foo.com", true},
...
func TestParseRequestURI(t *testing.T) {
	for _, test := range parseRequestURITests {
...
func TestResolveReference(t *testing.T) {
	...
	opaque := &URI{Scheme: "scheme", Opaque: "opaque"}
	for _, test := range resolveReferenceTests {
		base := mustParse(test.base)
		rel := mustParse(test.rel)
		url := base.ResolveReference(rel)
		if got := url.String(); got != test.expected {
			t.Errorf("URI(%q).ResolveReference(%q)\ngot  %q\nwant %q", test.base, test.rel, got, test.expected)
		}
		// Ensure that new instances are returned.
		if base == url {
			t.Errorf("Expected URI.ResolveReference to return new URI instance.")
		}
		// Test the convenience wrapper too.
		url, err := base.Parse(test.rel)
		if err != nil {
			t.Errorf("URI(%q).Parse(%q) failed: %v", test.base, test.rel, err)
		} else if got := url.String(); got != test.expected {
			t.Errorf("URI(%q).Parse(%q)\ngot  %q\nwant %q", test.base, test.rel, got, test.expected)
		} else if base == url {
			// Ensure that new instances are returned for the wrapper too.
			t.Errorf("Expected URI.Parse to return new URI instance.")
		}
		// Ensure Opaque resets the URI.
		url = base.ResolveReference(opaque)
		if *url != *opaque {
			t.Errorf("ResolveReference failed to resolve opaque URI:\ngot  %#v\nwant %#v", url, opaque)
		}
		// Test the convenience wrapper with an opaque URI too.
		url, err = base.Parse("scheme:opaque")
		if err != nil {
			t.Errorf(`URI(%q).Parse("scheme:opaque") failed: %v`, test.base, err)
		} else if *url != *opaque {
			t.Errorf("Parse failed to resolve opaque URI:\ngot  %#v\nwant %#v", opaque, url)
		} else if base == url {
...
func TestURIErrorImplementsNetError(t *testing.T) {
...
var (
	_ encodingPkg.BinaryMarshaler   = (*URI)(nil)
	_ encodingPkg.BinaryUnmarshaler = (*URI)(nil)
)
...
func TestInvalidUserPassword(t *testing.T) {
	...
	if got, wantsub := fmt.Sprint(err), "uri: invalid userinfo"; !strings.Contains(got, wantsub) {
...
func TestRejectControlCharacters(t *testing.T) {
	...
	for _, s := range tests {
		_, err := Parse(s)
		const wantSub = "uri: invalid control character in URI"
	...
	if _, err := Parse("http://foo.com/ctl\x80"); err != nil {
		t.Errorf("error parsing URI with non-ASCII control byte: %v", err)
	}
}
...
func TestMarshalText(t *testing.T) {
	tests := []struct {
		...
		{
			&URI{Scheme: "https", Host: "example.com", Path: "/path", Fragment: "fragment"},
			"https://example.com/path#fragment",
		},
		{
			&URI{Scheme: "https", Host: "example.com", Path: "/path", RawQuery: "query"},
			"https://example.com/path?query",
		},
		{
			&URI{Scheme: "https", Host: "example.com", Path: "/path", RawQuery: "query", Fragment: "fragment"},
			"https://example.com/path?query#fragment",
		},
	}
	for _, test := range tests {
		out, err := test.in.MarshalText()
		if err != nil {
			t.Errorf("MarshalText(%q) returned error: %v", test.in, err)
		}
		if string(out) != test.out {
			t.Errorf("MarshalText(%q) = %q, want %q", test.in, out, test.out)
		}

		out2, err := json.Marshal(test.in)
		if err != nil {
			t.Errorf("Marshal(%q) returned error: %v", test.in, err)
		}
		if string(out2) != fmt.Sprintf("%q", test.out) {
			t.Errorf("Marshal(%q) = %q, want %q", test.in, out2, test.out)
		}
	}
}
...
func TestUnmarshalText(t *testing.T) {
	tests := []struct {
		...
		{
			"http://golang.org",
			&URI{Scheme: "http", Host: "golang.org"},
		},
		{
			"http://golang.org/",
			&URI{Scheme: "http", Host: "golang.org", Path: "/"},
		},
		{
			"http://golang.org/path",
			&URI{Scheme: "http", Host: "golang.org", Path: "/path"},
		},
		{
			"http://golang.org/path/",
			&URI{Scheme: "http", Host: "golang.org", Path: "/path/"},
		...
		{
			"http://golang.org/path?query",
			&URI{Scheme: "http", Host: "golang.org", Path: "/path", RawQuery: "query"},
		},
		{
			"http://golang.org/path?query#fragment",
			&URI{Scheme: "http", Host: "golang.org", Path: "/path", RawQuery: "query", Fragment: "fragment"},
		},
		{
			"http://golang.org/path?query#fragment",
			&URI{Scheme: "http", Host: "golang.org", Path: "/path", RawQuery: "query", Fragment: "fragment"},
	...
	for _, test := range tests {
		...
		var u URI
		if err := u.UnmarshalText([]byte(test.in)); err != nil {
			t.Errorf("UnmarshalText(%q) = %v, want nil", test.in, err)
			continue
		}
		if !reflect.DeepEqual(&u, test.out) {
			t.Errorf("UnmarshalText(%q) = %v, want %v", test.in, &u, test.out)
		}
		...
		if err := json.Unmarshal(b, &u2); err != nil {
			t.Errorf("json.Unmarshal(%q) = %v, want nil", b, err)
			continue
		}
		if !reflect.DeepEqual(&u2, test.out) {
			t.Errorf("json.Unmarshal(%q) = %v, want %v", b, &u2, test.out)
		}
	}
}
